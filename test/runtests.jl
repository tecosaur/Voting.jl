...
@testset "Plurality" begin
    @test elect(Plurality(), convert.(PluralityBallot, ranked_2)) ==
...
@testset "InstantRunoff" begin
    @test elect(InstantRunoff(), ranked_2) ==
...
@testset "RankedPairs" begin
    @test elect(RankedPairs(), ranked_1) ==
        Voting.RankedPairsResult(1,
                                 [(2 => 4, 11),
                                  ...
                                  (3 => 4, 3),
                                  (1 => 3, 1)])
    @test elect(RankedPairs(), ranked_2) ==
